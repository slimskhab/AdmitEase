...
function Ebooks(props) {
  ...
  const fetchEbooks = () => {
    if (userData.userType === "Admin") {
      ...
      axios
        .get(`${process.env.REACT_APP_BACKEND}/ebook/${userData.id}`)
        .then((res) => {
          setEbooks(res.data.ebooks);
        })
        .catch((err) => {});
  ...
  const handleDelete = (id) => {
    axios
      .post(`${process.env.REACT_APP_BACKEND}/ebook/delete`, {
        ebookId: id,
      })
      .then((res) => {
        toast({
  ...
  const handleSubmitButton = (e) => {
    if (validateFields()) {
      ...
      axios
        .post(`${process.env.REACT_APP_BACKEND}/ebook/add`, formData)
        .then((res) => {
          toast({
            title: "Added.",
      ...
      <div style={{ width: "80%", background: "#F8F8F8" }}>
        <TopBar />
          ...
          >
            ...
            >
              <span>{userData.userType==="Admin"?"All":"My"} E-books</span>{" "}
              <FontAwesomeIcon
                icon={faPlus}
                onClick={onOpen}
                style={{
            ...
            {ebooks.length === 0 && (
              <span style={{ padding: "20px" }}>No E-books</span>
            )}
            {ebooks.map((e, i) => {
              return (
                <Card
