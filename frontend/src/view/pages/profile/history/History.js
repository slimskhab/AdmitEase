...
function History() {
  ...
  const handleDelete = (index) => {
    axios
    .post(`${process.env.REACT_APP_BACKEND}/history/delete`, {
      historyId: history[index].id,
    })
    .then((res) => {
      toast({
        title: "Deleted.",
        description: "Academic history deleted!",
        status: "success",
        duration: 9000,
        isClosable: true,
      });
        ...
        >
          <FormControl isInvalid={isInstitutionNameError}>
            ...
            {isInstitutionNameError && (
              <FormErrorMessage>Institution name is required.</FormErrorMessage>
            )}
          </FormControl>
          <br></br>

          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <FormControl isInvalid={isDegreeTypeError} style={{ width: "45%" }}>
            ...
            >
              ...
              />
              {isDegreeFieldError && (
                <FormErrorMessage>Degree Field is required.</FormErrorMessage>
              )}
            </FormControl>
          </div>
          <br></br>
          ...
          <div style={{ alignSelf: "flex-end", marginBottom: 20 }}>
            ...
            <Button colorScheme="teal" size="md" onClick={handleSaveButton}>
              Save
            </Button>
          </div>
        </div>
